...
export function FeatCard({ title, description, children, className = "" }: FeatCardProps) {
  return (
    ...
    >
      <div className="z-10 flex flex-col gap-1.5 font-sans">
      ...
      <div className="relative mt-2 flex-1 w-full rounded-[14px] overflow-hidden border border-border/55 bg-neutral-50 dark:bg-neutral-950/50">
        {children}
      </div>
    </motion.div>
  );
...
export function Card2() {
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between p-3">
            ...
            <div key={i} className="flex-1 h-[76px] relative select-none">
              ...
              >
                ...
                <div className="flex flex-col min-w-0 font-mono">
                  <span className="text-[8px] text-muted-foreground/80 uppercase tracking-widest leading-none">{s.label}</span>
                  <span className="text-base font-bold text-foreground leading-none mt-1.5 tracking-tight">{s.value}</span>
                  <div className="flex items-center gap-1.5 mt-2">
                    <span className={`text-[8px] font-bold ${s.trend.startsWith("+") ? "text-emerald-500" : "text-rose-400"
                      }`}>
                      {s.trend}
                    </span>
                    <span className="text-[8px] text-muted-foreground/50">prev</span>
                  </div>
                </div>
...
export function Card3() {
  ...
  return (
    <div className="w-full h-full relative flex items-center justify-center overflow-hidden p-3 min-h-[220px]">
      {logs.map((l, i) => {
...
export function Card4() {
  ...
  return (
    <div className="w-full h-full flex flex-col sm:flex-row gap-5 py-2 px-3">
      ...
      {/* Thin divider */}
      <div className="hidden sm:block w-px bg-border/30 self-stretch shrink-0" />
...
const CARDS = [
  ...
  },
  {
    ...
    description: "Semantic search across documents, codebase, and team discussions.",
    visual: <Card4 />,
...
export function AgentBentoGrid({ className }: AgentBentoGridProps) {
  return (
    <div className={cn("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-5xl mx-auto px-4", className)}>
      {CARDS.map((card, idx) => (
        <FeatCard
          key={idx}
          title={card.title}
